        ...
        items.append(
            {
                ...
                "channel": str(getattr(row, "messenger_channel", None) or "telegram"),
                ...
                "dead_lettered_at": row.dead_lettered_at.isoformat() if getattr(row, "dead_lettered_at", None) else None,
                "degraded_reason": (
                    row.last_error if str(getattr(row, "failure_class", "") or "") == "misconfiguration" else None
                ),
        ...
        items.append(
            {
                ...
                "channel": str(getattr(row, "channel", None) or "telegram"),
                "attempts": int(row.attempts or 0),
                "attempt_no": int(getattr(row, "attempt_no", row.attempts or 0) or 0),
                "created_at": row.created_at.isoformat() if row.created_at else None,
                "next_retry_at": row.next_retry_at.isoformat() if row.next_retry_at else None,
                "last_error": row.last_error,
                "failure_class": getattr(row, "failure_class", None),
