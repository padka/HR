...
This module is the canonical home for HTTP instrumentation. It is implemented as
ASGI middleware (instead of BaseHTTPMiddleware) to:
- reduce overhead under load
- handle client disconnects during response streaming without noisy tracebacks

Design constraints:
- low-cardinality route labels (prefer route template, sanitize raw paths early)
- minimal per-request overhead when METRICS_ENABLED=0
- optional diagnostic headers in non-production only
"""
...
class HTTPMetricsMiddleware:
    ...
    async def __call__(self, scope: dict[str, Any], receive: Callable, send: Callable) -> None:
        ...
        try:
            ...
            # Cache markers.
            cache_hits: list[perf_context.CacheEvent] = []
            if ctx is not None:
