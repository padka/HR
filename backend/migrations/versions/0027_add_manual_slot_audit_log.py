...
def upgrade(conn: Connection) -> None:
    ...
    if dialect == "postgresql":
        ...
        conn.execute(sa.text("""
            CREATE INDEX ix_manual_slot_audit_logs_slot_id ON manual_slot_audit_logs (slot_id)
        """))
        conn.execute(sa.text("""
            CREATE INDEX ix_manual_slot_audit_logs_candidate_tg_id ON manual_slot_audit_logs (candidate_tg_id)
        """))
        conn.execute(sa.text("""
            CREATE INDEX ix_manual_slot_audit_logs_created_at ON manual_slot_audit_logs (created_at)
        """))
    else:
        # SQLite
        conn.execute(sa.text("""
            CREATE TABLE manual_slot_audit_logs (
                ...
                slot_id INTEGER NOT NULL REFERENCES slots(id) ON DELETE CASCADE,
                candidate_tg_id INTEGER NOT NULL,
                recruiter_id INTEGER NOT NULL REFERENCES recruiters(id) ON DELETE CASCADE,
                city_id INTEGER REFERENCES cities(id) ON DELETE SET NULL,
                slot_datetime_utc TIMESTAMP NOT NULL,
                slot_tz VARCHAR(64) NOT NULL,
                purpose VARCHAR(32) NOT NULL DEFAULT 'interview',
                custom_message_sent BOOLEAN NOT NULL DEFAULT 0,
                custom_message_text TEXT,
                admin_username VARCHAR(100) NOT NULL,
                ip_address VARCHAR(45),
                user_agent VARCHAR(255),
...
def downgrade(conn: Connection) -> None:  # pragma: no cover - rollback support
    ...
    conn.execute(sa.text("DROP TABLE IF EXISTS manual_slot_audit_logs"))
