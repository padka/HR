#!/usr/bin/env bash
# Development helper to run the Admin UI locally (dev-only).
...
echo "==================================================="
echo "Starting Admin UI (development mode)"
echo "==================================================="

# Warn about conflicting BOT_TOKEN from the shell (common cause of NullBot/dummy token)
if [ -n "${BOT_TOKEN:-}" ]; then
...
exec uvicorn backend.apps.admin_ui.app:app --host 0.0.0.0 --port 8000 --reload
