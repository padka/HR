      ...
      {testSections.length > 0 && (
        <div className="cd-tests-grid">
          {testSections.map((section) => {
            const latestAttempt =
              section.history?.[0] ??
              (section.details?.questions?.length || section.details?.stats
                ? {
                    id: 0,
                    completed_at: section.completed_at,
                    raw_score: section.details?.stats?.raw_score,
                    final_score: section.details?.stats?.final_score,
              ...
              <div key={section.key} className="cd-test-card">
                ...
                {latestAttempt ? (
                  <button
                    ...
                    onClick={() => onOpenAttemptPreview(section.title, latestAttempt)}
                  >
                    Посмотреть ответы
                  </button>
                ) : section.report_url ? (
