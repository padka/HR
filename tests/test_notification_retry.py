...
@pytest.mark.asyncio
@pytest.mark.notifications
async def test_reschedule_requested_recruiter_marks_failed_when_candidate_missing(monkeypatch):
    store = InMemoryStateStore(ttl_seconds=60)
    manager = StateManager(store)
    dummy_bot = SimpleNamespace()
    configure(dummy_bot, manager)
    ...
    async def fake_send(_bot, _method, _correlation_id):
        raise AssertionError("send should not be called when candidate context is missing")
